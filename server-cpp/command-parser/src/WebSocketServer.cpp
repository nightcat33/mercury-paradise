...
#define ME_UNIVERSAL_BUF_SIZE 256
...
public:
	void handleRequest(HTTPServerRequest& request, HTTPServerResponse& response)
	{
        ...
        std::ifstream f("resources/html/demo.html", std::ifstream::in);
		...
		try
		{
			...
			do
			{
				...
				app.logger().information(Poco::format("Frame received (length=%d, flags=0x%x).", n, unsigned(flags)));
                                std::string dat = std::string(buffer);
		                app.logger().information(dat);
								std::string result;

									result = handler->handle(dat);
								
                                app.logger().information(result);
                                ws.sendFrame(result.c_str(), result.size(), flags);
			}
